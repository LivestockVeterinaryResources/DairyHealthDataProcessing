...
# # Read Parquet file
production_data <- read_parquet(production_data)
...
pregnant_counts_by_period <- animals_repro_gc3 %>%
  ...
  left_join(bred_counts_by_period)
...
preg_risk_long <- pregnant_counts_by_period %>%
  ...
  mutate(pregnancy_risk = total_pregnancies / total_eligible,
         ai_sub_risk = total_breedings / total_eligible) %>%
...
preg_risk_lact_group <- preg_risk_long %>%
  ...
  summarise(
    total_breedings = sum(total_breedings, na.rm = TRUE),
  ...
  mutate(pregnancy_risk = total_pregnancies / total_eligible,
         ai_sub_risk = total_breedings / total_eligible) %>%
  replace_na(list(pregnancy_risk = 0, ai_sub_risk = 0)) 

...
test_hfr<-deno_base%>%
  ...
  select(id_animal, id_animal_lact, date_birth, lact_number,  date_fresh, date_sold, date_died, date_archive, date_elig_start, date_elig_end)
...
for (i in seq_along(calendar$date_calendar)){
  ...
  df4<-df%>%
   ...
   df5<-df%>%
    group_by(animal_location_event_list, lact_group_5, breed, date_ref) |> 
    summarize(ct_animals = sum(eligible))%>%
    ungroup()|> 
    rename(`Lactation Group` = lact_group_5)
  ...
  # 
  #final df---------------------------
  df10<-bind_rows(df2, df3, df4, df5)%>%
    distinct()

...
This graphs shows fundamental events that occured at each "location" for past 5 years
...
ggplot(health_details%>%filter(disease %in% list_dz[[i]]))+
  geom_bar(aes(x = event,y = ct_row,
                fill = location_event), stat = 'identity')+
    facet_grid(protocols~., scales = 'free')+

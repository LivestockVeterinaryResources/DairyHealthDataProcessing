...
qc_lactations <- deno_base %>%
  select(id_animal_lact, location_lact_list, location_list, date_lact_start, qc_missing_lact_param, qc_complete_lactation, qc_flag_lactation_duration) %>%
  ...
  group_by(location_list, lact_year_start, parameter)%>%
  mutate(total_lactations = n_distinct(id_animal_lact))%>%
  ungroup()%>%
  group_by(location_list, lact_year_start, parameter, total_lactations, outcome)%>%
...
ggplot(qc_lactations%>%filter(parameter %in% choose_param))+
  ...
  facet_grid(parameter~location_list, scale = 'free')+
  theme_bw()+
...
df_period_summary<-df_period%>%
  group_by(location_list, lact_group, period_start, period_end, days_in_period, period_complete, qc_days_elig)%>%
  filter(days_elig>0)%>%
...
df_period_summary<-df_period%>%
  group_by(location_list, lact_group, period_start, period_end, days_in_period,  qc_days_elig)%>%
  filter(days_elig>0)%>%
  summarize(days_elig = sum(days_elig, na.rm = TRUE), 
            count_lactations = n_distinct(id_animal_lact), 
            count_animals = n_distinct(id_animal))%>%
...
fxn_plot_deno_types<-function(deno_type){
  ...
  deno_type_long<-deno_type%>%
    select(period_start, count_animals, count_lactations, animal_periods,  lact_group, location_list)%>%
...
p1<-ggplot(deno_type_long)+
  ...
  facet_grid(location_list~lact_group)

p2<-ggplot(deno_type)+
  geom_smooth(aes(x = count_animals, y = count_lactations, color = lact_group, fill = lact_group, group = lact_group))+
  facet_wrap(.~location_list, scales =  'free')+
  theme_bw()+

...
herd_deno_dim<-deno_dataframe_phase%>%
  ...
  group_by(time_period, dim_categories, `Lactation Group`, location_lact_list )%>%
  fxn_summarize_denos()
...
ggplot(deno_dataframe_phase)+
  geom_point(aes(x = date_ref, y = ct_lacts, color = dim_categories))+
  ylim(c(0, NA))+
...
```{r}
waldo::compare(
sort(unique(colnames(gerard_herd_deno_dim))),
sort(unique(colnames(herd_deno_dim)))
)

waldo::compare(
sort(unique(colnames(gerard_herd_deno_season))),
sort(unique(colnames(herd_deno_month)))
)

waldo::compare(
sort(unique(colnames(gerard_herd_deno))),
sort(unique(colnames(herd_deno) ))
)

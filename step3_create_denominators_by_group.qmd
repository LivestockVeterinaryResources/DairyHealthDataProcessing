...
denominator_granular<-deno_dataframe%>%
  ...
  select(animal_location_event_list, `Lactation Group`, ct_animals, contains('date'))
...
fxn_create_deno<-function(time_period){
  denominator_granular%>%
    group_by(animal_location_event_list, `Lactation Group`, {{time_period}})%>%
...
ggplot(denominator_time_period)+
  geom_linerange(aes(y = animal_count_mean, xmin = time_period_start, xmax = time_period_end ))+
...
for (i in seq_along(calendar$date_calendar)){
  ...
  df3<-df%>%
    ...
    group_by(animal_location_event_list, lact_group, date_ref, dim_categories) |> 

...
## Test Comparison to Old Gerard Deno

```{r, message = FALSE}
gerard_deno<-read_parquet('data/intermediate_files/herd_denominators.parquet')%>%
  rename(ct_animals = count_animals)%>%
  mutate(date_ref = lubridate::ymd(paste0(time_period, '/01/01')))

waldo::compare(deno_dataframe%>%
                 select(`Lactation Group`,  ct_animals, date_ref, everything())%>%
                 filter(date_ref>=lubridate::ymd('2025/01/01')),
               gerard_deno%>%
                 select(`Lactation Group`,  ct_animals, date_ref, everything()))

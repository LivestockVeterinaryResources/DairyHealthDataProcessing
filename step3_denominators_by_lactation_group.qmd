...
Mulitple denominator types are generated.  
...
These value are calculated for each lactation group within the time_period, as well as for each dim group within lactation group. (see section below on denominator types) 
...
for (i in seq_along(calendar$date_calendar)){
  ...
  df_pre<-set_deno_base_type%>%
    ...
    mutate(day_of_phase = case_when( #this is the day of phase at the reference time_period start
      lact_number==0~as.numeric(date_ref_start-date_birth), 
      lact_number>0~as.numeric(date_ref_start-date_fresh), 
      TRUE~as.numeric(NA)
    ))
  
  df<-df_pre%>%filter(elig_days>0)
  ...
  df6<-df%>%
    #filter(elig_days>0)%>%
    fxn_assign_day_of_phase_group(cut_by_days = params$cut_by_days, 
                                  top_cut = params$top_cut, 
                                  top_cut_hfr = params$top_cut_hfr)%>% #modify the cut points if desired
  ...
  df7<-df%>%
    ...
    fxn_assign_day_of_phase_group(cut_by_days = params$cut_by_days, 
                                  top_cut = params$top_cut, 
                                  top_cut_hfr = params$top_cut_hfr)%>% #modify the cut points if desired
    group_by(location_lact_list, lact_group_repro, day_of_phase_group, date_ref_start, date_ref_end) |>
    summarize(ct_animal_days_elig = sum(elig_days),
              ct_animals = n_distinct(id_animal),
              ct_animal_lactations = n_distinct(id_animal_lact))%>%
    ungroup()|>
  ...
  df8<-df%>%
    ...
    fxn_assign_day_of_phase_group(cut_by_days = params$cut_by_days, 
                                  top_cut = params$top_cut, 
                                  top_cut_hfr = params$top_cut_hfr)%>% #modify the cut points if desired
    group_by(location_lact_list, lact_group, day_of_phase_group, date_ref_start, date_ref_end) |>
    summarize(ct_animal_days_elig = sum(elig_days),
              ct_animals = n_distinct(id_animal),
              ct_animal_lactations = n_distinct(id_animal_lact))%>%
    ungroup()|>
  ...
  df9<-df%>%
    ...
    fxn_assign_day_of_phase_group(cut_by_days = params$cut_by_days, 
                                  top_cut = params$top_cut, 
                                  top_cut_hfr = params$top_cut_hfr)%>% #modify the cut points if desired
    group_by(location_lact_list, lact_group_5, day_of_phase_group, date_ref_start, date_ref_end) |>
    summarize(ct_animal_days_elig = sum(elig_days),
              ct_animals = n_distinct(id_animal),
              ct_animal_lactations = n_distinct(id_animal_lact))%>%
    ungroup()|>
...
ggplot(deno_final%>%filter(!(str_detect(deno_type, 'dop_'))))+
  ...
  theme_bw()+
  labs(title = 'Animal time_periods (ct_animal_time_periods)')
```
...
These settings can be modified if you wish by adjusting the the options in the `fxn_assign_dop`.

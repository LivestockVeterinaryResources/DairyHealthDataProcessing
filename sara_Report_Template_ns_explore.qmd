...
<!-- get_api_key<-'WPT2DPQQ96BTQLWBTRDQFF3AR#' -->
<!-- #https://weather.visualcrossing.com/VisualCrossingWebServices/rest/services/timeline/olsburg%20ks?unitGroup=us&key=WPT2DPQQ96BTQLWBTRDQFF3AR&contentType=json' -->
...
```{r repro eligible animals}
test_cows<-c('13916_07/19/11', '15584_09/08/12')
test_cows<-sort(unique(animals$id_animal))

# Ensure lact_number > 0 and only repro events that affect elig. 
#Do we want this?? what about animals that freshend and didn't get bred nor dnb?
repro_elig <- repro_data %>%
  filter(id_animal %in% test_cows)%>%
  filter(#lact_number > 0,
         event %in% c("PREG", "ABORT", "BRED", "OK", "OPEN", "DNB", "BULLPEN")
         )%>%
...
```{r get fresh dates}
# set up animal data - Cows
animals_fresh <- animal_lactations %>% 
    filter(id_animal %in% test_cows)%>%
  filter(lact_number > 0) %>%
  #filter(lact_number != 32) |> 
  filter(date_archive >= date_start_repro | is.na(date_archive)) %>% #exclude lactations archived before start date
  select(id_animal_lact, date_fresh, date_archive) #date enrolled?
...
animals_repro2 <- all_periods %>%
  left_join(animals_fresh, by = c("id_animal_lact")) %>%
  
     #***This seems dangerous . . . why is it needed? ***
...
latest_events <- latest_preg_events %>%
  left_join(
    latest_abort_events %>% 
      select(id_animal_lact, period_start_date, latest_abort_event),  # Only keep relevant columns from latest_abort_events
    by = c("id_animal_lact", "period_start_date")
  )

# Merge the latest PREG and ABORT event dates back into animals_repro
animals_repro_test <- animals_repro2 %>%
  left_join(
    latest_events %>%
...
#***Should  R=="C" be sorted out at the begining? A pregnancy due to a previous breeding essentially had no effect other than labor and semen expense***

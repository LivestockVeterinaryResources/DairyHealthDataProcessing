...
for (i in seq_along(calendar$date_calendar)){
  ...
  df2<-df%>%
    group_by(animal_location_event_list, lact_group_basic, breed_simple, date_ref) |>  #add custom grouping variable here
    summarize(ct_animals = sum(eligible))%>%
    ungroup() |> 
    rename(`Lactation Group` = lact_group_basic)
  
  df3<-df%>%
    group_by(animal_location_event_list, lact_group, breed_simple, date_ref) |> #add custom grouping variable here
    summarize(ct_animals = sum(eligible))%>%
    ungroup()|> 
    rename(`Lactation Group` = lact_group)
  
  df4<-df%>%
    group_by(animal_location_event_list, lact_group_5, breed_simple, date_ref) |> #add custom grouping variable here
    summarize(ct_animals = sum(eligible))%>%
    ungroup()|> 
...
ggplot(deno_dataframe)+
  ...
  ylim(c(0, NA))+
  facet_grid(animal_location_event_list~breed_simple)+
  theme_bw()+
...
denominator_granular<-deno_dataframe%>%
  ...
  select(animal_location_event_list, `Lactation Group`, breed_simple, ct_animals, contains('date'))

write_parquet(denominator_granular, 'data/intermediate_files/denominator_calendar_time_custom_breed.parquet')
...
fxn_create_deno<-function(time_period){
  denominator_granular%>%
    group_by(animal_location_event_list, `Lactation Group`, breed_simple, {{time_period}})%>% #add custom grouping variable here
...
denominator_time_period<-bind_rows(denominator_year, denominator_season, denominator_month, denominator_week)%>%
  select(`Lactation Group`, breed_simple, contains('time_period'), contains('animal'), contains('floordate'))%>% #add custom grouping variable here
...
write_parquet(denominator_time_period, 'data/intermediate_files/denominator_time_period_custom_breed.parquet')
...
ggplot(denominator_time_period)+
  geom_linerange(aes(y = animal_count_mean, 
                     ...
                     color = breed_simple))+
  facet_grid(`Lactation Group`~animal_location_event_list)
...
for (i in seq_along(calendar$date_calendar)){
  ...
  df3<-df%>%
    ...
    group_by(animal_location_event_list, lact_group, breed_simple, date_ref, dim_categories) |> #add custom grouping variable here
...
write_parquet(deno_dataframe_phase, 'data/intermediate_files/denominator_phase_time_custom_breed.parquet')
...
p<-ggplot(deno_dataframe_phase%>%filter(breed_simple %in% list_custom_var[[i]]))+
  ...
  ylim(c(0, NA))+
  facet_grid(animal_location_event_list~`Lactation Group`)+

...
This graph deomnostrates the proportion of events each month by event type.  Exploration of this allows detection of changes in system
...
ggplot(trend_events_by_location%>%
  ...
  geom_bar(aes(x = floordate_month,
               y = count_rows, fill = event), stat = 'identity', position = position_fill())+
    facet_wrap(location_event ~., scales = 'free', nrow = 2)+
  scale_fill_viridis_d()+
  theme_bw()+
...
p<-ggplot(health_details%>%filter(disease %in% list_dz[[i]]))+
  geom_bar(aes(x = protocols,y = ct_row,

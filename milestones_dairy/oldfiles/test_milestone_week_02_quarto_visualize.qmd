...
This shows you that we are currently in the main project directory. If I tell it to look for a file it will begin in this location.
...
Write some code in this chunk that will read in the file named "animals.parquet", and name it "animals" in your environment. (Hint: remember that paths generally need to be in quotes)
...
Run the code chunk below to see a plot. Your task is to recreate this plot. Don't worry too much if you can't get the name of the Example Herd to show up, it is a bonus challenge. ![Image Caption](images/milestone02.png){width="4in" height="3in"}
...
ggplot(animals%>%filter(age_left>365))+
  ...
  geom_text(aes(x = 3000, y = 1000, label = paste0(unique(animals$location_first))))

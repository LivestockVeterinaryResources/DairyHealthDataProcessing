...
Now that you can tidy data, quickly pivot tables to reformat the way you need, and then join tables to others, you are both unstopable . . . and dangerous.

In this lesson we will explore the standard denominator file creation.  

The chunk below calls a script which will create a standard denominator file and save it in data/intermediatefiles folder with a file name of `denominator_by_lact_group_time_periodXXX.parquet`.  The "XXX" in the file name represents the number of days within each time period used to denominator calculations.

When you run the chunk below it will 
-  generate a denominator file with a time period of 365 days
-  read that file into your environment and name it deno365.

```{r initial denominator generation}
...
There are multiple denominator types to choose from in this file. 
Write some code in the chunk below to list the names of the different values in the deno_type variable
...
You will notice that there are 2 basic types
 -  lactation groups (begin with "lact_")
 -  day of phase plus lactation group (bein with "dop_")
...
You should pivot the columns: "ct_animal_time_periods",  "ct_animals" , "ct_animal_lactations"
Name your new pivoted dataframe `deno365_long`.
...
write_rds(deno365_long, 'data_milestones/solutions_week_05_pivotlonger.rds')
```
...
Modify the code below (***change the numeric values for the execute_params***) to create different time periods or day of phase cut points. 
Then describe the new denominator you generated using a graph, table or both.
...
2) use you new join skills to join `animal_lactations.parquet` to `animals.parquet` and create your own denominator from scratch that improves your ablility to answer a question about your data in milestone 4.

3) Do anything else you think makes sense for a question that you need a denominator to answer. (for some questions the best denominator is a subset of events pivoted wider)

NOTE: This can be a challenging task.  If you get stuck:

...
1.  Read in **events_formatted.parque**t from the **../data/intermediate_files**, and name it **events_formatted. Then filter so you only have the LAME events**
2.  create a new variable named **floordate_month**. The logic to create this variable is: **floor_date(date_event, 'months')**
3.  arrange the data by 3 variables: the animal, the event, and then the event date. In this data set these variables are named: id_animal, event, event_date
...
5.  select all the variables you see in the solution chunk
...
1.  Create a new column named `lame_type` that represents a more specific lameness diagnosis.  Feel free to do something simple like differentiate trims from actual lame events.

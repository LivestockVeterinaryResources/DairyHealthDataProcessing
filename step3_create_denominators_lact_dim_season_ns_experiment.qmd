...
date_end_deno <- data_pull_max
date_start_deno <- data_pull_max-365

label_deno <- as.character(year(date_end_deno))
...
# to get cows with dates in window
deno <- animal_lactations |>
  left_join(animals%>%select(id_animal, date_left)) |> #nora added this as the "real" date left . . . which is slightly different than date_archive_max
  ...
  #***Lets discuss if it works to do this by joing to animals, it keeps it cleaner to not calculate the same value in 2 locations, see addition on line 56***
  group_by(id_animal) |>
  mutate(date_not_elig = max(date_archive_max)) |>
  ungroup() |>
  ...
  filter(is.na(date_archive) | date_archive >=  date_start) |> 
   ...
   # filters cows fresh before start date
  filter(date_fresh <= date_start & date_archive >= date_start |
           date_fresh <= date_start & is.na(date_not_elig) | 
  # filters cows fresh after start date
    date_fresh >= date_start & date_archive <= date_end |
           date_fresh >= date_start & is.na(date_not_elig)
         )
...
waldo::compare(denominators, denominators2)


list_animals_original<-sort(unique(denominators$id_animal))
list_animals_nora<-sort(unique(denominators2$id_animal))

waldo::compare(list_animals_original, list_animals_nora) #final animals included are the same, and we didn't create a date_left that has a different definition than the one in the animals table
